...
fun_desc_t cmd_table[] = {
    ...
    {cmd_cwd,"cwd","current directory"},
    {cmd_cd,"cd","change directroy"},
};
    ...
    if (fundex >= 0) {
      ...
      if(pid<0){
        ...
        if(x==0){
          ...
          for(int i=0;i<tokens_get_length(tokens);i++){
            arg[i]=(char*)malloc(200);
            strcpy(arg[i],tokens_get_token(tokens,i));
          }
          arg[tokens_get_length(tokens)]=NULL;
          execv(arg[0],arg);
          exit(1);
        }
        else{
          ...
          while(count<=x){
            ...
            if(pid==-1){
              exit(1);
                ...
                arg[0]=(char*)malloc(200);
                strcpy(arg[0],tokens_get_token(tokens,2*count));
                arg[1]=NULL;
